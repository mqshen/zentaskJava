...
(function($, undefined) {
	...
	Calendar.prototype = {
		...
		init: function ( type, element, options ) {
			...
			var prevMonth = new Month(this.year, this.currentMonth - 1);
			var nextMonth = new Month(this.year, this.currentMonth + 1);
		...
		prevMonth: function() {
			var self = this;
			this.monthOjb.data('slider').prevElement();
			var changeIndex = this.currentIndex + 1;
			if(changeIndex > 2) 
				changeIndex = 0;
			this.currentMonth -= 1;
			...
			var month = new Month(this.year, this.currentMonth - 1, this.months, this.options.selectFun);
			
			var calendarBody = $('#day-slider-' +  changeIndex, this.$element);
			calendarBody.children().remove();
			
			var prevMonthObj = $(month.renderDays());
			
			$('td', prevMonthObj).bind('click.calendar-day', function(event) {
				return self.dayClick(event);
			});
			
			calendarBody.append(prevMonthObj);
			--this.currentIndex;
		...
		nextMonth: function() {
			var self = this;
			this.monthOjb.data('slider').nextElement();
			var changeIndex = this.currentIndex - 1;
			if(changeIndex < 0)
				changeIndex = 2
			this.currentMonth += 1;
			...
			var nextMonthObj = $(month.renderDays());
			
			$('td', nextMonthObj).bind('click.calendar-day', function(event) {
				return self.dayClick(event);
			});
			
			calendarBody.append(nextMonthObj);
			++this.currentIndex;
